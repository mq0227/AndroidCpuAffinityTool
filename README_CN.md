...
### 自动 CPU 检测
应用会自动读取每个核心的最大频率，智能识别并分组核心（小核/中核/大核）。这意味着它支持多种 CPU 架构：

- **骁龙 8 Gen2/Gen3**（1+4+3 或 1+3+4 布局）
- **骁龙 8 Gen1**（1+3+4 布局）
- **骁龙 888/870**（1+3+4 布局）
- **联发科天玑** 系列
- **其他 ARM big.LITTLE 架构**

### 优化适配
虽然应用可以在大多数已 Root 的安卓设备上运行，但 CPU Boost 配置专门针对以下设备进行了调优：
- **中兴（ZTE）** 系列
- **努比亚（Nubia）** 系列
- **红魔（RedMagic）** 游戏手机（红魔 8 Pro、9 Pro 等）

Boost 参数针对努比亚内核（如 `cpufreq_ctrl`、`walt` 调度器），但核心亲和性功能在所有设备上通用。
...
应用会自动检测你的 CPU 布局。以骁龙 8 Gen2 为例：
...
对于其他 CPU，应用会读取 `/sys/devices/system/cpu/cpuX/cpufreq/cpuinfo_max_freq` 并根据最大频率自动分组核心。
